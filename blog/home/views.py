...
#from rest_framewoimport MySQLdb
    ...
    if request.method == 'GET':
        ...
        if(tuple_response[0] == "1"):
            ...
            if "Duplicate entry" in str(tuple_response[1]):
                ...
                context = {'account_error': "Account already register"}
                return HttpResponse(template.render(context, request))
            
            elif (tuple_response[1] == "error"):
                template = loader.get_template('home/login_register.html')
                context = {'account_error': "Username and password not match"}
                return HttpResponse(template.render(context, request))
...
def login_controller(request):
    ...
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='9872476129Mm', db='grocery_store')
    cur = conn.cursor()
...
def tracking_controller(request):
    ...
    query = 'SELECT destination,current_lat,current_long,start_lat,start_long,status,track_id  FROM tracking where track_id = "' + str(tracking_number) +  '" '   
    #return HttpResponse(query)
    cur.execute(query)
...
def shoppingcart(request):
    # render simple static page
    template = loader.get_template('home/shoppingcart.html')
    context = {}
    return HttpResponse(template.render(context, request))
